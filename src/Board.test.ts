...
it('create a board which is made up of 64 squares arranged in 8 rows and 8 columns', () => {
  ...
  expect(Board.SQUARES_PER_LINE).toBe(8)
  expect(board.rows).toBe(Board.SQUARES_PER_LINE)
  expect(board.columns).toBe(Board.SQUARES_PER_LINE)
  expect(board.squares).toBe(Board.SQUARES_PER_LINE ** 2)
})
